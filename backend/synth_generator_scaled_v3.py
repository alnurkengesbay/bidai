...
# -*- coding: utf-8 -*-
"""
synth_generator_scaled_v3.py
----------------------------
v3: добавлен CSV с "истинным" количеством объектов (N_true) и метаданными кадра,
а также поддержка списков-манестов (файлы с именами) для выбора подмножеств спрайтов/фонов.
Основано на v2 (масштаб зерна в мм, случайный кроп фона, debug, Poisson/alpha blending).

CSV-формат (append):
id,image_path,label_path,N_true,is_negative,size,frame_cm,px_per_cm,grain_mm_min,grain_mm_max,seed
"""
...
def load_images(dir_path, with_alpha=False, name_whitelist=None):
    paths = []
    for ext in ('*.png','*.jpg','*.jpeg','*.webp'):
        paths.extend(Path(dir_path).glob(ext))
    items = []
    for p in paths:
        if name_whitelist is not None and p.name not in name_whitelist:
            continue
    ...
    if not items:
        raise SystemExit(f"[ERR] Не нашёл подходящих изображений в {dir_path}")
    return items
...
def read_list(txt_path):
    ...
    p = Path(txt_path)
    if not p.exists():
        raise SystemExit(f"[ERR] Файл списка не найден: {txt_path}")
...
def per_sprite_color_jitter(rng, sprite_rgba):
    ...
    a = sprite_rgba[:, :, 3:4]
    alpha = float(np.clip(rng.normal(1.0, 0.03), 0.95, 1.05))  # минимальный разброс яркости
    beta  = float(np.clip(rng.normal(0.0, 4.0), -6, 6))    # минимальный разброс контраста
    rgb = np.clip(rgb*alpha + beta, 0, 255)
    gamma = float(np.clip(rng.normal(1.0, 0.02), 0.97, 1.03))  # минимальный разброс гаммы
...
def global_augs(rng, img):
    ...
    # Минимальное JPEG сжатие (высокое качество)
    q = int(np.clip(rng.normal(95, 3), 90, 98))
...
def main():
    ...
    ap.add_argument("--list_sprites", type=str, default="")
    ap.add_argument("--list_backgrounds", type=str, default="")
    ap.add_argument("--csv", type=str, default="")
    args = ap.parse_args()

    rng, pyrand = set_seed(args.seed)

    wl_spr = read_list(args.list_sprites) if args.list_sprites else None
    wl_bg  = read_list(args.list_backgrounds) if args.list_backgrounds else None

    sprites = load_images(args.sprites, with_alpha=True, name_whitelist=wl_spr)
    backs   = load_images(args.backgrounds, with_alpha=False, name_whitelist=wl_bg)
    ...
    if csv_path:
        ...
        if not exists:
            writer.writerow(["id","image_path","label_path","N_true","is_negative","size","frame_cm","px_per_cm","grain_mm_min","grain_mm_max","seed"])
    ...
    for i in range(args.n):
        ...
        if rng.random() < max(0.0, min(1.0, args.neg_prob)):
            ...
            if writer:
                writer.writerow([f"im_{i:05d}", str(im_path), str(lb_path), 0, 1, args.size, args.frame_cm, px_per_cm, args.grain_mm_min, args.grain_mm_max, args.seed])
            continue
        ...
        if writer:
            writer.writerow([f"im_{i:05d}", str(im_path), str(lb_path), placed, 0, args.size, args.frame_cm, px_per_cm, args.grain_mm_min, args.grain_mm_max, args.seed])
